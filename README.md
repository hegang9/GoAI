...
| `[rabbitmqConfig]`     | RabbitMQ 地址、凭证和 vhost；迁移期间保留的主链路、五档旧延迟重试、最终 DLQ、本地重试、prefetch 与发布确认超时                                                                                                                                                |
...
持久化层将 Message、Target 和重试次数展开为 `DelayTaskPO` 字段，Headers 使用 JSON 保存；PO 恢复领域对象时统一通过领域构造函数校验，非法 JSON、目标类型或任务状态会作为错误返回，不会静默生成无效任务。

重试计数区分两种语义：`RetryAttempt` 表示业务消息当前消费重试次数，`DispatchAttempts` 表示 Poller 抢占任务并尝试转交 Level MQ 的累计次数；数据库暂时沿用 `retry_times`、`attempts` 列名以避免无必要迁移。

应用层 `DelayService` 是普通延迟和消费重试的统一入口：`Schedule` 将 60 秒内任务发布到对应 Level、将更长任务交给 MySQL Repository，并拒绝超过 7 天的任务；`ScheduleRetry` 按 consumer group 静态策略计算下一次 RetryAttempt、目标时间和确定性 schedule ID，再复用 `Schedule` 完成长短延迟分流。
...
RabbitMQ `GroupConsumer` 使用消费者组专用 Channel、显式 prefetch 和手动 ACK 读取业务 Queue，并将 Delivery 同时恢复为 Handler 使用的聊天消息和延迟重试使用的稳定业务消息；Body 与 AMQP 属性中的 message ID、Topic 和 `x-retry-attempt` 会在调用 Handler 前完成一致性校验。业务成功后直接 ACK；瞬时失败由注入函数调用 `ScheduleRetry`，永久错误、损坏消息和重试耗尽则通过当前组的 routing key 可靠发布到组级 DLQ。只有重试任务或 DLQ 已收到 Broker confirm 且未被 `mandatory` 退回后才 ACK 原 Delivery；调度失败、DLQ 发布失败和 Abort 均保留未 ACK 状态并关闭消费 Channel。该组件尚未接入 bootstrap，运行时链路仍未启动。
