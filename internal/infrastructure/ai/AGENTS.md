...
- `auto` 是统一自动编排模型：分层记忆恢复 + planner 检索决策 + RetrievalModifier 一次性检索增强 + Eino ADK Agent（默认 MCP 工具集，模型自主 native function calling）。
...
- 检索是 pre-generation 上下文准备，必须在进入 ADK Agent 前由 `RetrievalModifier` 一次性完成；循环中途最后一条可能是工具结果，重复检索既昂贵又会改错位置。
- 上下文压缩优先复用 Eino 官方 Summarization/Reduction；会话摘要与核心记忆通过领域端口持久化，不把请求级状态写入可并发复用的 Agent 实例。
